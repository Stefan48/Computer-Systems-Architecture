...
__global__ void swap_arrays(int *a, int *b, int *c, int start)
{
...
int main(void) {
    ...
    cudaMalloc((void**)&device_array_c, BUF_32M * sizeof(int));
    ...
    int blocks_no = 1 << 11, block_size = BUF_2M / blocks_no;
    ...
    {
		swap_arrays<<<blocks_no, block_size>>>(device_array_a, device_array_b, device_array_c, i);
