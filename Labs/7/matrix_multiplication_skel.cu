...
__global__ void matrix_multiply_simple(float *ma, float *mb, float *mc, size_t width)
{
	...
	unsigned int block = blockIdx.x * gridDim.y + blockIdx.y;
	unsigned int pos = block * blockDim.x * blockDim.y + threadIdx.x * blockDim.y + threadIdx.y;
	unsigned int row = pos / width;
	unsigned int col = pos % width;
	
	//int row = blockIdx.y * blockDim.y + threadIdx.y;
	//int col = blockIdx.x * blockDim.x + threadIdx.x;
	//mc[row * width + col] = 1.0f;
...
//DEBUG
/*
void print_matrix(FILE *f, std::vector<float> v, int width)
{
	for (int i = 0; i < width; ++i)
	{
		for (int j = 0; j < width; ++j)
		{
			fprintf(f, "%f ", v[i * width + j]);
		}
		fprintf(f, "\n");
	}
	fprintf(f, "\n");
...
int main(void)
{
	...
	//print_matrix(f, host_a, n);
	//print_matrix(f, host_b, n);
	...
	for(int i = 0; i < num_launches; ++i) {
		...
		/*if (i == 0)
		{
			cudaMemcpy(&host_c[0], device_c, sizeof(float) * n * n, cudaMemcpyDeviceToHost);
			print_matrix(f, host_c, n);
		}*/	
	}
